...
class Submit extends CI_Model {
	...
	public function avatar($submitID, $file) {
		$sql = 'UPDATE `submit` SET `avator` = ? WHERE `submitID` = ?';
		return $this->db->query($sql, [$file, $submitID]);
	}

	public function apply($submitID, $file) {
		$sql = 'UPDATE `submit` SET `apply` = ? WHERE `submitID` = ?';
		return $this->db->query($sql, [$file, $submitID]);
	}
